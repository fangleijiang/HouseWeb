...
<html>
  ...
  <body>
   ...
   <jsp:include page="user_center_left.jsp"></jsp:include>
      ...
      <table id="mytable">
      <tr style="background-color: #98C9CE"><td>id</td><td>标题</td><td>小区</td><td>厅室</td><td>预定日期</td><td>联系人</td><td>手机号码</td><td>预订人</td><td>预订人手机号码</td><td>删除</td></tr>
      <c:forEach items="${list}" var="cart">
      <tr><td>${cart.id}</td><td><a href="findHouseInfoByid.action?id=${cart.houseInfo.id}">${cart.houseInfo.title}</a></td><td>${cart.houseInfo.houseName }</td><td>${cart.houseInfo.householdType}</td><td>${cart.date }</td><td>${cart.houseInfo.user.username }</td><td>${cart.houseInfo.user.phoneNumber }</td><td>${cart.user.username }</td><td>${cart.user.phoneNumber }</td><td><a href="admin/deleteCart.action?id=${cart.id }">删除</a></td></tr>
      </c:forEach>
      </table>
      </div>
    </div></div>
