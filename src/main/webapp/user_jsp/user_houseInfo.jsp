...
<html>
  ...
  <body>
   ...
   <jsp:include page="user_center_left.jsp"></jsp:include>
      ...
      <tr><td>${house.id}</td><td><a href="findHouseInfoByid.action?id=${house.id}">${house.title}</a></td><td>${house.publishDate }</td><td><a href="user/deleteHouseInfoById.action?id=${house.id}">删除</a></td></tr>
      </c:forEach>
      </table>
      </div>
    </div></div>
